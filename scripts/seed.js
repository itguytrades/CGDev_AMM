...
async function main() {
  ...
  // Fetch Dapp Token
  const cgd = await ethers.getContractAt('Token', config[chainId].dapp.address)
  ...
  transaction = await cgd.connect(deployer).transfer(investor1.address, tokens(10))
  ...
  transaction = await cgd.connect(deployer).transfer(investor3.address, tokens(10))
  await transaction.wait()
  ...
  transaction = await cgd.connect(deployer).approve(amm.address, amount)
  await transaction.wait()
  ...
  transaction = await cgd.connect(investor1).approve(amm.address, tokens(10))
  await transaction.wait()
  ...
  // Investor approves all tokens
  transaction = await cgd.connect(investor3).approve(amm.address, tokens(10))
  await transaction.wait()
